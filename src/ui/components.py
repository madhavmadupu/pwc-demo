...
Streamlit components shared across pages: tech banner, sidebar, result display.
"""
...
def render_tech_banner() -> None:
    """Render the technology stack metric banner."""
    st.markdown("---")
...
def render_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        st.write(
            f"**Vector Search:** "
            f"{'✅ Enabled' if settings.vector_search_endpoint else '⚠️ In-Memory'}"
        )
        ...
        if validation.get("checks"):
            for check in validation["checks"]:
                icon = (
                    ...
                    else "⚠️" if check.get("status") == "warning"
                    else "❌"
                )
...
def render_summary(result: dict) -> None:
    ...
    if summary and summary.get("title"):
        ...
        if summary.get("overall_status"):
            ...
            st.write(
                f"**Overall Status:** {status_color} "
                f"{summary['overall_status'].replace('_', ' ').title()}"
            )
    else:
...
def render_rules_applied(result: dict) -> None:
    """Render the RAG rules applied section.
